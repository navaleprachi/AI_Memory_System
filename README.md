...
User Message
    │
    ▼
Save to DB ──► Chunk + Embed ──► Vector Store (pgvector)
    │                                    │
    │              ┌─────────────────────┘
    ▼              ▼
Score importance   Vector search (top 20 chunks)
(background)            │
    │              3-factor ranking:
    │              • Relevance  (50%) — cosine similarity
    │              • Recency    (30%) — 7-day exponential decay
    │              • Importance (20%) — LLM-scored 0–1
    │                    │
    │              Top 5 chunks injected into prompt (1500 token budget)
    │                    │
    ▼                    ▼
Check compression   LLM response (GPT-4o-mini)
threshold                │
    │              Save + embed reply
    ▼
20 msgs → L1 Summary (bullet points)
5 L1s  → L2 Summary (merged bullet points)
```
...
```
conversations
    └── messages (role, content, importance_score, is_compressed)
            └── chunks (content, embedding VECTOR(1536), source_type)
    └── summaries (level, content, source_ids, parent_id)
            └── chunks (content, embedding VECTOR(1536), source_type='summary')
```
